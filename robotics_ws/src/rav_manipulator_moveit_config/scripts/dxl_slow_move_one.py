...
try:
    print(f"=== SLOW MOVE XM540 | ID {args.dxl_id} | delta {args.delta:+d} ===")
    ...
    if target < 0 or target > 4095:
        print(f"ABORTADO: alvo {target} fora do range 0–4095.")
        sys.exit(1)

    print(f"Posição inicial: {pos0}")
    print(f"Alvo:            {target}")
    print(f"Velocidade:      {args.velocity}")
    print("Movimento começando em 2 segundos...")
    time.sleep(2.0)
    ...
    time.sleep(0.2)
    w4(port, packet, args.dxl_id, ADDR_GOAL_POSITION, target, "goal position")
    ...
    for _ in range(80):
        ...
        print(f"present={pos:4d} | target={target:4d} | error={pos-target:+4d} | moving={moving}")
